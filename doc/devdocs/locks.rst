...
Below are all of the locks that exist in the system and the mechanism in use
that prevents them from being a potential deadlock.
...
The following are definitely leaf locks (level 1), and must not try to acquire any other lock:
   ...
   * safepoint
       ...
       Note that there are safepoint when doing allocation, enabling / disabling GC,
       entering / restoring exception frame, and taking / releasing locks.
...
The following is a level 4 locks, which can only recurse to acquire level 1, 2, or 3 locks:

   * Method->writelock

     but note that this is violated by staged functions!

The following are level 5 locks, which can only recurse to acquire to acquire locks at lower levels:

   * MethodTable->writelock

     but note that this is violated by staged functions!

The following is a proposed level 6 locks, which can only recurse to acquire locks at lower levels:

   * staged

       this theoretical lock would create a priority inversion from the `method->writelock` (level 4),
       but only prohibiting only running any staging function in parallel
       (thus allowing temporary release of the MethodTable and Method locks)

The following is a level 7 locks, which can only recurse to acquire locks at lower levels:

   * codegen

The following is an almost root locks (level end-1), meaning only the root look may be held when trying to acquire it:

   * typeinf

       this one is perhaps one of the most tricky ones, since type-inference can be invoked from many points

The following is the root locks, meaning no other lock shall be held when trying to acquire it:
...
* staged
    ...
    this theoretic lock would create a priority inversion such that the Method and MethodTable write locks could be released
    by ensuring that no staging functions can run in parallel allow this level 6 lock to protect staged function conflicts (a level 4 operation)

    fix: create it

* typecache

    this only protect cache lookup and insertion but it doesn't make the lookup-and-construct atomic
